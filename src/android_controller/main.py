...
def main(
    ...
    max_iterations: int = typer.Option(
        50, "--max-iterations", "-n", help="Maximum number of observe-think-act cycles"
    ),
    ...
    verbose: bool = typer.Option(
        False, "--verbose", "-v", help="Show detailed output including LLM thoughts"
    ),
    mcp: bool = typer.Option(
        False,
        "--mcp",
        help="Run as MCP (Model Context Protocol) server for use with LLM clients",
    ),
):
    """🤖 LLM-controlled Android device automation via ADB.

    If no command is provided, starts an interactive chat session.
    Use --mcp to run as an MCP server for integration with LLM clients.
    """
    ...
    # MCP server mode
    if mcp:
        from android_controller.mcp_server import main as mcp_main

        mcp_main(serial=serial)
        return
...
def screenshot(
    output: str = typer.Option(
        "screenshot.png", "--output", "-o", help="Output file path"
    ),
    serial: Optional[str] = typer.Option(
...
def control(
    ...
    if not match:
        ...
        console.print(
            "[dim]Expected format: command_name(arg1=value1, arg2=value2)[/dim]"
        )
        raise typer.Exit(1)
    ...
    try:
        ...
        console.print(
            "[dim]Available: tap, tap_sequence, swipe, input_text, press_home, press_back, press_enter, launch_app, wait[/dim]"
        )
        raise typer.Exit(1)
    except Exception as e:
        console.print(f"[bold red]❌ Error:[/bold red] {e}")
